...
// Phase 7/8: Enforce login
requireLogin('orders.php');

$orderId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$order = getOrderById($orderId);

// Security: Ensure order exists and belongs to the logged-in user
...
<div class="container mb-3rem">
    <div class="flex-between mb-2">
        <div>
            <a href="orders.php" class="text-muted-sm td-n flex-center-gap-0-5 mb-0-5">
            ...
            <p class="text-muted-sm">Placed on <?php echo date('F j, Y \a\t g:i A', strtotime($order['created_at'])); ?></p>
        </div>
                                ...
                                <div class="flex-gap-1 mb-0-5">
                                    <?php foreach ($item['variant'] as $type => $value): ?>
                                        <span class="text-muted-sm fs-0-875">
                                            <strong class="color-text-main"><?php echo ucfirst($type); ?>:</strong> <?php echo $value; ?>
                                        </span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                            <p class="text-muted-sm">₹<?php echo number_format($item['price']); ?> × <?php echo $item['quantity']; ?></p>
                        </div>
                        <div class="text-right">
                            <p class="font-700 color-primary fs-1-1">₹<?php echo number_format($item['price'] * $item['quantity']); ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <div class="flex-col-gap-2">
            ...
            <div class="card bg-card border-all">
                ...
                <div class="flex-col-gap-1">
                    ...
                    <div class="flex-between">
                        <span class="text-muted">Shipping (<?php echo htmlspecialchars($order['shipping_type']); ?>)</span>
                    ...
                    <div class="border-t-2 mt-0-5 pt-1 flex-between items-center">
                        ...
                        <span class="font-800 fs-1-5 color-primary">₹<?php echo number_format($order['final_amount']); ?></span>
                    </div>
                </div>
            ...
            <div class="card">
                ...
                <?php if ($order['address']): ?>
                    ...
                    <p class="text-muted-sm lh-1-6">
                        ...
                        <?php if ($order['address']['address_line2']): ?>
                            <?php echo htmlspecialchars($order['address']['address_line2']); ?><br>
                        <?php endif; ?>
                        <?php echo htmlspecialchars($order['address']['city']); ?>, <?php echo htmlspecialchars($order['address']['state']); ?> - <?php echo htmlspecialchars($order['address']['postal_code']); ?><br>
                        <?php echo htmlspecialchars($order['address']['country']); ?>
                    </p>
                    <div class="mt-1 flex-col-gap-0-25">
                        <p class="text-muted-sm"><strong>Phone:</strong> <?php echo htmlspecialchars($order['address']['phone']); ?></p>
                        <p class="text-muted-sm"><strong>Email:</strong> <?php echo htmlspecialchars($order['address']['email']); ?></p>
                    </div>
                <?php else: ?>
